...
export class ProductsPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.womenDressLink = page.locator("#Women").getByRole("link", { name: "Dress" });
    this.womenTopsLink = page.locator("#Women").getByRole("link", { name: "Tops" });
    this.womenSareeLink = page.locator("#Women").getByRole("link", { name: "Saree" });

    this.menCategoryLink = page.getByRole("link", { name: "Men" });
    this.menTShirtsLink = page.locator("#Men").getByRole("link", { name: "Tshirts" });
    this.menJeansLink = page.locator("#Men").getByRole("link", { name: "Jeans" });

    this.kidsCategoryLink = page.getByRole("link", { name: "Kids" });
    this.kidsDressLink = page.locator("#Kids").getByRole("link", { name: "Dress" });
    this.kidsTopsAndShirtsLink = page.locator("#Kids").getByRole("link", { name: "Tops & Shirts" });

    this.brandLinks = page.locator(".brands-name a");

    this.productCards = page.locator(".product-image-wrapper");
    this.addToCartDefaultButtons = page.locator(".productinfo .add-to-cart");
    this.addToCartOverlayButtons = page.locator(".product-overlay .add-to-cart");
    this.viewProductLinks = page.locator(".choose a");

    this.addedToCartModal = page.locator("#cartModal");
    this.continueShoppingButton = page.getByRole("button", { name: "Continue Shopping" });
    this.viewCartLink = this.addedToCartModal.getByRole("link", { name: "View Cart" });
  }
  ...
  async getProductNameByIndex(index: number): Promise<string> {
    return (
      (await this.productCards.nth(index).locator(".productinfo p").textContent()) ?? ""
    );
  }
